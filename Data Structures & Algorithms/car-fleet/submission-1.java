class Solution {
    public int carFleet(int target, int[] position, int[] speed) {
        ...
        //c
            ...
            for(int i=0;i<n;i++){
                map.put(position[i],speed[i]); //logN
            }
            //NlogN

            Deque<Double> stack=new ArrayDeque<>(); // ts: concurrentLinkedDeque/LinkedBlockingQue : putLast
            for(Map.Entry<Integer,Integer> e: map.entrySet()){ // in-order-traversal is O(N)
                double newCarRemainingTimeToReach= (double)(target-e.getKey())/e.getValue(); //O(1)
