class KthLargest {
    ...
    */
    ...
    public int add(int val) {
        //Both the removal and insertion operations take O(log k) time.
        if(minHeap.size()<k)minHeap.offer(val); //log K
        else if(val> minHeap.peek()){
            minHeap.poll(); //remove root -> logK
            minHeap.offer(val);// log K
        }
