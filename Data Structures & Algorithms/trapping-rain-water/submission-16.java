class Solution {
    public int trap(int[] height) {
        ...
        we cannot store water at boundary hence h[0]and h[legth-1] cant store water
    ...
    private int approach1(int[] height){
        ...
        int res=0;
        // Imp: we can skip left and right boundary too
        for(int i=0;i<height.length;i++){
    ...
    private int approach3(int[] height){
        // two ptr + greedy
        // we know at any point limiting factor is lmax or rmax hence we try to improve it
        // if lmax is smaller need to find better lmax to increase cap
        // if lmax is small move l++ 
        int n=height.length;
        int l=1, r=n-2;
        int lmax=height[0], rmax=height[n-1];
        int res=0;
         // height = [3, 0, 2]
         // l=1, r=1 we want process it hence l==r must be checked too
        while(l<=r){
            if(lmax< rmax){
                lmax=Math.max(lmax,height[l]);
                res+=lmax-height[l];
